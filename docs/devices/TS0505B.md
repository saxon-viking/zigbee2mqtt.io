...
## PAIRING / FACTORY RE-SET: howto
TuYa TS0505B - mine was branded ENSHINE, no other ID on it. Amazon purchase.
Switch the bulb ON (1second) -- OFF (2secs) -- ON(1second) -- OFF(2secs) -- ON, the bulb will blink slowly like it is breathing. It will breathe (available to pair) for 1 minute if it fails to connect. Repeat the process if it does not work the first time.
